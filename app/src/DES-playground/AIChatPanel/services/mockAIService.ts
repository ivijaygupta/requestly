...
export const parseIntent = (message: string): ParsedIntent => {
  ...
  if (
    ...
    const resourceMatch = lowerMessage.match(/(\w+)\s+(?:collection|api|crud|endpoints)/i);

    if (forMatch) {
      resourceName = forMatch[1];
    } else if (resourceMatch && !["crud", "a", "the", "generate", "create", "make"].includes(resourceMatch[1])) {
  ...
  const methodMatch = lowerMessage.match(/\b(get|post|put|patch|delete|head|options)\b/i);
  const hasRequestKeyword =
    lowerMessage.includes("request") || lowerMessage.includes("endpoint") || lowerMessage.includes("api");
  ...
  if (methodMatch || (hasRequestKeyword && hasCreateKeyword)) {
    const method = (methodMatch?.[1]?.toUpperCase() || "GET") as CreateRequestPayload["method"];
    ...
    const resourceMatch = lowerMessage.match(/(?:to|for)\s+(?:get|fetch|retrieve|list|create|update|delete)?\s*(\w+)/i);
 ...
 */
export const generateResponse = async (intent: ParsedIntent): Promise<AIServiceResponse> => {
  await simulateDelay();
    ...
    case "create_collection": {
      ...
      return {
        message:
          ...
          payload.requests.map((r) => `• **${r.method}** \`${r.url}\` - ${r.name}`).join("\n") +
...
function parseModification(message: string): ModifyRequestPayload {
  ...
  if (
    ...
    const headerMatch = message.match(/(\w+[-\w]*)\s*(?:header|:)\s*(?:to\s+)?["']?([^"'\n]+)["']?/i);
    if (headerMatch) {
    ...
    } else if (lowerMessage.includes("content-type") || lowerMessage.includes("content type")) {
  ...
  if (lowerMessage.includes("method")) {
    ...
    if (methodMatch && HTTP_METHODS.includes(methodMatch[1].toUpperCase() as any)) {
